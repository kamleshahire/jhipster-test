(function() {
    ...
    function jhiAlertErrorController ($scope, AlertService, $rootScope) {
        ...
        var cleanHttpErrorListener = $rootScope.$on('iTasteApp.httpError', function (event, httpResponse) {
            var i;
            ...
            case 400:
                var errorHeader = httpResponse.headers('X-iTasteApp-error');
                var entityKey = httpResponse.headers('X-iTasteApp-params');
                if (errorHeader) {
